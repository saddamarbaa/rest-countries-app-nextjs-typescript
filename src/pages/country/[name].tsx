...
interface MapDispatchProps {
  getSingleCountry: (name: string | string[]) => void;
}
...
function Index({ listState, getSingleCountry }: PropsType) {
  const router = useRouter();
  const [name, setName] = useState('');
  const { country, getSingleIsLoading } = listState;

  useEffect(() => {
    if (name) getSingleCountry(name);
  }, [name]);
  ...
  return (
    <>
      ...
      {/* TODO */}
      <div>{/* integrate Interactive Map */}</div>
    </>
  );
}
